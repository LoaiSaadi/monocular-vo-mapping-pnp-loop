...
# ============================================================
# What this script provides (per HW):
# - Real-time keypoints overlay (OpenCV window)
# - Real-time trajectory + 3D map viewer (Pangolin if available, else Open3D, else none)
# - Before/after match filtering visualization (optional OpenCV window)
# - Outputs folder:
#   outputs/map_points.ply
#   outputs/trajectory_raw.txt
#   outputs/trajectory_corrected.txt
#   outputs/metrics.csv
#
# Notes:
# - Intrinsics K are estimated from image size (fx = 0.9*w, fy=fx, cx=w/2, cy=h/2)
# - Monocular scale is unknown: we normalize translation direction for VO integration
# - Loop-closure is a lightweight "snap" correction (pose correction only; no full BA)
...
class FRAME:
    Id: int
    Timestamp: float
    # Raw pose from VO integration (camera->world)
    Pose_raw: np.ndarray  # 4x4 camera->world
...
# PnP localization
# ----------------------------
def localize_pnp(world_map: Map, frame: FRAME, K: np.ndarray, min_corr: int = 50):
...
class PangolinViewer:
    def __init__(self, w=1024, h=768):
        ...
        try:
            ...
            self.s_cam = pangolin.OpenGlRenderState(
                pangolin.ProjectionMatrix(w, h, 500, 500, w / 2.0, h / 2.0, 0.1, 10000),
    ...
    def update(self, points_xyz: np.ndarray, points_rgb: np.ndarray, traj_xyz: np.ndarray):
        ...
        if points_xyz is not None and len(points_xyz) > 0:
            ...
            for (x, y, z), (b, g, r) in zip(points_xyz, points_rgb):
...
def main(dataset_dir: str,
    ...
    if viewer is None and viewer_mode in ("auto", "open3d"):
        ...
        if viewer.ok:
            ...
            if viewer_mode == "open3d":
                ...
                print("[INFO] Open3D not available -> using only OpenCV 2D windows (still saving PLY).")
    ...
    try:
        for i in range(min(max_frames, len(rgb_list))):
            ...
            m_total = 0
            m_inl = 0
            epi_med_all = float("nan")
            epi_med_in = float("nan")
            pnp_m = 0
            pnp_inl = 0
            rmse_b = float("nan")
            rmse_a = float("nan")
            ...
            if prev_frame is not None:
                ...
                if len(good) >= 20:
                    ...
                    if F is not None and fmask is not None:
                        ...
                        se_all = sampson_errors(F, pts1_all.astype(np.float64), pts2_all.astype(np.float64))
                        se_in = se_all[fmask] if len(se_all) == len(fmask) else np.array([], dtype=np.float64)

                        epi_med_all = float(np.median(se_all)) if len(se_all) else float("nan")
                        epi_med_in = float(np.median(se_in)) if len(se_in) else float("nan")
                        ...
                        if inl >= 12:
                            ...
                            if E is not None:
                                try:
                                    ...
                                    T_c2_c1 = make_T(R, tn)
                                    T_w_c_raw = prev_frame.Pose_raw @ invert_T(T_c2_c1)
                                    frame.Pose_raw = T_w_c_raw.copy()
                                except cv2.error:
                                    pass
            ...
            if show_matches_window and (i % show_every) == 0 and (matches_before_img is not None or matches_after_img is not None):
                ...
                cv2.imshow("matches_before_after", np.vstack([top, bot]))

            T_w_c_corr = T_world_corr @ frame.Pose_raw
            ...
            if frame.Id != keyframe.Id and len(world_map.points) < 8000:
                T_w_c_kf_corr = T_world_corr @ keyframe.Pose_raw
            ...
            if (frame.Id % pnp_every) == 0 and len(world_map.points) >= 80:
                res = localize_pnp(world_map, frame, K, min_corr=50)
                if res is not None:
                    ...
                    T_world_corr = T_w_c_pnp @ invert_T(frame.Pose_raw)
            ...
            if (frame.Id % loop_every) == 0 and len(keyframes_for_loop) >= 3:
                ...
                if loop is not None:
                    ...
                    if kf is not None:
                        T_w_c_kf_corr = T_world_corr @ kf.Pose_raw
                        T_w_c_target = T_w_c_kf_corr @ invert_T(T_c_cur_c_kf)

                        T_world_corr_old = T_world_corr.copy()
                        T_world_corr = T_w_c_target @ invert_T(frame.Pose_raw)
                        world_delta = T_world_corr @ invert_T(T_world_corr_old)
            ...
            if viewer is not None and (i % show_every) == 0:
                if len(world_map.points) > 0:
                    xyz = np.vstack([p.point for p in world_map.points])
                    rgb = np.vstack([np.array(p.color_bgr, dtype=np.float64) for p in world_map.points])
                else:
                ...
                viewer.update(
                    xyz,
                    rgb,
                    np.vstack(corr_positions) if len(corr_positions) else np.zeros((0, 3), dtype=np.float64)
                )

                if isinstance(viewer, PangolinViewer) and viewer.should_quit:
                    break
            ...
            writer.writerow({
                ...
                "epi_med_all": epi_med_all,
                "epi_med_in": epi_med_in,
                "pnp_matches": pnp_m,
                "pnp_inliers": pnp_inl,
