...
# ✅ (1) Feature extraction + matching + show matches before/after filtering (OpenCV windows)
# ✅ (2) Epipolar Sampson error in TWO stages: (A) before geometry (8-point) (B) after geometry (RANSAC inliers)
# ✅ (3) Accumulated trajectory (raw + corrected)
# ✅ (4) 3D map via triangulation (PointCloud) + visualization + export to PLY
# ✅ (5) PnP relocalization every N frames
# ✅ (6) Reprojection RMSE before/after refinement
# ✅ (7) Optimizers: LM / GN / GD (Gradient Descent) / none
# ✅ (8) Loop closure detection + apply correction and show effect
# ✅ Pangolin real-time display improved: auto-scale + bigger far plane + bigger points + current pose marker
# ✅ VO stabilization: parallax-scaled translation + skip pose update when parallax too small
#
# Notes:
# - Intrinsics K estimated from image size: fx = 0.9*w, fy=fx, cx=w/2, cy=h/2
# - Monocular scale is unknown: we scale translation step using parallax proxy (pixels)
# - Loop closure is lightweight "snap" correction (no full pose-graph/BA)
...
class FRAME:
    Id: int
    Timestamp: float
    Pose: np.ndarray                  # 4x4 camera->world
    RotationMatrix: np.ndarray        # 3x3 (from Pose)
    TranslationVector: np.ndarray     # 3,  (from Pose)
    KeyPoints: list
...
def set_frame_pose(frame: FRAME, T_w_c: np.ndarray):
    frame.Pose = T_w_c.copy()
    frame.RotationMatrix = T_w_c[:3, :3].copy()
    frame.TranslationVector = T_w_c[:3, 3].copy()
...
# ----------------------------
# Pose refinement (GD numeric) - Gradient Descent
# ----------------------------
def refine_pose_gd_numeric(K: np.ndarray,
                           rvec: np.ndarray,
                           tvec: np.ndarray,
                           Xw: np.ndarray,
                           uv: np.ndarray,
    ...
    for _ in range(iters):
        ...
        grad = np.zeros((6, 1), dtype=np.float64)
        for j in range(6):
            rv2, tv2 = rvec.copy(), tvec.copy()
            if j < 3:
                rv2[j, 0] += eps
            else:
                tv2[j - 3, 0] += eps
            grad[j, 0] = (loss(rv2, tv2) - base) / eps
...
def localize_pnp(world_map: Map,
    ...
    map_desc = np.vstack([p.des for p in world_map.points])
    bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=False)
    ...
    if refine == "lm":
        ...
        rvec_ref, tvec_ref = refine_pose_gd_numeric(K, rvec, tvec, Xw_in, uv_in, iters=30, lr=1e-3)
    elif refine == "none":
        pass
    ...
    T_c_w = make_T(R, tvec_ref)  # world->camera
...
# ----------------------------
# Pangolin viewer (preferred by HW) - improved visibility
# ----------------------------
class PangolinViewer:
    def __init__(self, w=1024, h=768):
        ...
        try:
            ...
            # Bigger far plane to avoid clipping
            self.s_cam = pangolin.OpenGlRenderState(
                pangolin.ProjectionMatrix(w, h, 520, 520, w / 2.0, h / 2.0, 0.01, 1e6),
    ...
    def update(self, points_xyz: np.ndarray, points_rgb: np.ndarray, traj_xyz: np.ndarray):
        ...
        # Center around current pose
        center = traj[-1].copy() if len(traj) > 0 else (np.mean(pts, axis=0) if len(pts) > 0 else np.zeros(3))
        ...
        max_r = 1e-6
        if len(traj_disp) > 0:
            max_r = max(max_r, float(np.max(np.linalg.norm(traj_disp, axis=1))))
        if len(pts_disp) > 0:
            max_r = max(max_r, float(np.max(np.linalg.norm(pts_disp, axis=1))))
        target_scale = 3.0 / max_r
        self._smooth_scale = 0.9 * self._smooth_scale + 0.1 * target_scale
        s = self._smooth_scale

        if len(pts_disp):
            pts_disp = pts_disp * s
        if len(traj_disp):
            traj_disp = traj_disp * s
        ...
        if len(traj_disp) >= 2:
            ...
            # Current camera position marker (red), at origin due to centering
            gl.glPointSize(8.0)
            gl.glBegin(gl.GL_POINTS)
...
def main(dataset_dir: str,
    ...
    # Correction transform on top of raw poses (for PnP + loop closure)
    ...
    try:
        for i in range(min(max_frames, len(rgb_list))):
            ...
            frame = FRAME(
                ...
                Timestamp=ts,
                Pose=T_w_c_raw.copy(),
                RotationMatrix=T_w_c_raw[:3, :3].copy(),
                TranslationVector=T_w_c_raw[:3, 3].copy(),
                KeyPoints=kp,
                Descriptors=des,
                ImageGray=img_gray,
                ImageBgr=img_bgr
            ...
            if prev_frame is not None:
                ...
                if len(good) >= 20:
                    if show_matches_window and (i % show_every) == 0:
                        ...
                        matches_before_img = cv2.drawMatches(
                            prev_frame.ImageBgr, prev_frame.KeyPoints,
                    ...
                    if F is not None and fmask is not None:
                        ...
                        if show_matches_window and (i % show_every) == 0:
                            ...
                            matches_after_img = cv2.drawMatches(
                                prev_frame.ImageBgr, prev_frame.KeyPoints,
                                frame.ImageBgr, frame.KeyPoints,
                                inlier_matches, None,
                                flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS
                        ...
                        if inl >= 12:
                            ...
                            if E is not None:
                                try:
                                    ...
                                    # --- Stabilize motion: parallax-based step + skip low parallax ---
                                    parallax = float(np.median(np.linalg.norm(pts1_in - pts2_in, axis=1)))

                                    if parallax >= 1.0:
                                        step = float(np.clip(parallax / 30.0, 0.05, 1.0))
                                        tn = (t / max(np.linalg.norm(t), 1e-12)) * step

                                        T_c2_c1 = make_T(R, tn)  # c1 -> c2
                                        T_w_c_raw = prev_frame.Pose @ invert_T(T_c2_c1)
                                        set_frame_pose(frame, T_w_c_raw)
                                    # else: skip update (keep previous pose)
            ...
            if show_matches_window and (i % show_every) == 0 and (matches_before_img is not None or matches_after_img is not None):
                ...
                text2 = (
                    f"epi(desc) med={epi_med_desc:.3g} mean={epi_mean_desc:.3g} | "
                    f"epi(geom-in) med={epi_med_geom_in:.3g} mean={epi_mean_geom_in:.3g}"
                )
            ...
            if (frame.Id % pnp_every) == 0 and len(world_map.points) >= 80:
                ...
                if res is not None:
                    ...
                    # Warp map points + corrected trajectory history
            ...
            # Window 2 (OpenCV): 2D XZ trajectory
            ...
            # Viewer update: Pangolin pumped every frame, Open3D lighter
    ...
    # Save with optional recentering (nice for plots)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pnp-refine", default="lm", choices=["lm", "gn", "gd", "none"],
                        help="PnP optimization method (HW #7): lm, gn, gd, none.")
