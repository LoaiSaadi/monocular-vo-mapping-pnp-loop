...
from map import Map
from pangolin_viewer import PangolinViewer
...
def load_image_sequence(dataset_dir: str) -> List[Tuple[float, str]]:
    ...
    - Else, search ONE level down for */rgb.txt.
...
# ============================================================
# Reprojection error + pose refinement
# ============================================================
...
def add_points_from_pair(world_map: Map,
    ...
    x_kf = triangulate_dlt(p1, p2, pts1_n, pts2_n)
...
def compose_status_view(cur_bgr: np.ndarray,
    ...
    for k in keypoints:
        ...
        cv2.circle(vis_kp, (x, y), 1, (0, 255, 0), -1)
        ...
        for i in range(min(max_frames, len(seq))):
            ...
            if (frame.id % pnp_every) == 0:
                ...
                if res is not None:
                    ...
                    print(
                        f"[{frame.id}] PnP: matches={pnp_matches} inliers={pnp_inliers} "
            ...
            text_lines = [
                ...
                f"epi(geom) med={epi_med_geom:.3g} mean={epi_mean_geom:.3g}",
                f"pnp: matches={pnp_matches} inliers={pnp_inliers}  "
                f"rmse_before={pnp_rmse_before:.2f}px rmse_after={pnp_rmse_after:.2f}px"
            ]
